"""
Valid URL examples:

bytebank.com/cambio
bytebank.com.br/cambio
www.bytebank.com/cambio
www.bytebank.com.br/cambio
http://www.bytebank.com/cambio
http://www.bytebank.com.br/cambio
https://www.bytebank.com/cambio
https://www.bytebank.com.br/cambio

Example of Invalid URL

https://bytebank/cambio
https://bytebank.naoexiste/cambio
ht://bytebank.naoexiste/cambio
"""
import re


def __main__():
    # Using "[abc]", we say that the pattern must contain either "a", or "b", or "c", using "(abc)", we say that the
    # pattern
    # must contain exactly the sequence "abc".
    # Follow this model "https://www.bytebank.com.br/cambio"
    url = "https://www.bytebank.com.br/cambio"
    url_pattern = re.compile("(http(s)?://)?(www.)?bytebank.com(.br)?/cambio")

    # The "match()" function verify if the string match exactly with the specified pattern
    match = url_pattern.match(url)

    if not match:
        raise ValueError("The URL is not valid")

    print("The URL is valid")


if __name__ == "__main__":
    __main__()
